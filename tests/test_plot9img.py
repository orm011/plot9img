...
def test_PIL_render():
    ...
    annotations_df = pd.DataFrame([{'x1': 100, 'x2':225, 'y1':100, 'y2':300, 'label':'a box'}])
    gimg = (ggimg(image=im, data=annotations_df) 
            + geom_rect(aes(xmin='x1', xmax='x2', ymin='y1', ymax='y2', color='label'), fill=None)
